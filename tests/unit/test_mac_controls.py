...
class TestSetVolume:
    ...
    def test_set_volume_clamps_to_zero(
        self, mock_platform_darwin, mock_subprocess_run
    ):
    ...
    def test_set_volume_non_darwin_logs_warning(
        self, mock_platform_linux, caplog
    ):
        """Test that non-macOS platform logs warning and returns."""
        set_volume(50)
        assert "only supported on macOS" in caplog.text
    
    def test_set_volume_missing_osascript_raises(
        self, mock_platform_darwin
    ):
    ...
    def test_set_volume_subprocess_error_raises(
        self, mock_platform_darwin
    ):
